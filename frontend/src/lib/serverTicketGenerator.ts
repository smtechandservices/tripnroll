...
export const generateTicketPDFBuffer = async (
    ...
    if (user?.profile?.brand_logo) {
        const brandLogoUrl = user.profile.brand_logo.startsWith('http')
            ? user.profile.brand_logo
            : `${process.env.NEXT_PUBLIC_API_URL || ''}${user.profile.brand_logo}`;
        const brandBase64 = await fetchImageAsBase64(brandLogoUrl, authToken);
        if (brandBase64) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    doc.text('Reservations (24/7)', W - 10, 19, { align: 'right' });
    if (user?.username) {
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7.5);
        doc.text(user.username.toUpperCase(), W - 10, 25, { align: 'right' });
    }
